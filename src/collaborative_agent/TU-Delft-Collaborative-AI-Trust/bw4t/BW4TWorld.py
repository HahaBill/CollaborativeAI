...
DEFAULT_WORLDSETTINGS: dict = {
    ...
    # the range with which agents detect other objects (walls, doors, etc.)
    'other_sense_range':  np.inf,
    'agent_memory_decay': 5,  # we want to memorize states for seconds / tick_duration ticks
    # true if walls block vision. Not sure if this works at all.
    'fov_occlusion': True
...
class BW4TWorld:
    ...
    def __init__(self, agents: List[dict], worldsettings: dict = DEFAULT_WORLDSETTINGS):
        ...
        self._builder.add_room(top_left_location=(
            0, 0), width=world_size[0], height=world_size[1], name="world_bounds")
    ...
    def world_size(self):
        ...
        nr_room_rows = np.ceil(
            worldsettings['nr_rooms'] / worldsettings['rooms_per_row'])
        ...
        world_height = nr_room_rows * worldsettings['room_size'][1] + (
            nr_room_rows + 1) * worldsettings['hallway_space'] + worldsettings['nr_blocks_needed'] + 2
    ...
    def _addBlocks(self, room_locations):
        ...
        for room_name, locations in room_locations.items():
            for loc in locations:
                ...
                prob = min(
                    1.0, self._worldsettings['average_blocks_per_room'] / len(locations))
    ...
    def _addRooms(self):
        ...
        for room_nr in range(self._worldsettings['nr_rooms']):
            ...
            room_locations[room_name] = get_room_locations(
                room_top_left, self._worldsettings['room_size'][0], self._worldsettings['room_size'][1])
    ...
    def get_room_loc(self, room_nr):
        ...
        room_y = int(1 + self._worldsettings['hallway_space'] * (
            row + 1) + row * self._worldsettings['room_size'][1] + 1)
    ...
    def _addDropOffZones(self, world_size):
        ...
        # once for off by one, another for world bound
        y = world_size[1] - 1 - 1
